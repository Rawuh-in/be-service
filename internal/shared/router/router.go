...
func NewRouter(g *guestHandler.GuestHandler, e *eventHandler.EventHandler, p *projectHandler.ProjectHandler) http.Handler {
	...
	r.HandleFunc("{project_id}/events/list/{user_id}", e.ListEvent).Methods(http.MethodGet)
	r.HandleFunc("{project_id}/events/{event_id}/{user_id}", e.DetailEvent).Methods(http.MethodGet)
	r.HandleFunc("{project_id}/events/{event_id}/{user_id}", e.UpdateEvent).Methods(http.MethodPut)
	r.HandleFunc("{project_id}/events/{event_id}/{user_id}", e.DeleteEvent).Methods(http.MethodDelete)
